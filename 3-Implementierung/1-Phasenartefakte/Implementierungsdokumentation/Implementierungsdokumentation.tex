...
\newglossaryentry{GEF}{
	name = {Grafical Editing Framework},
	description = {Ein GUI Framework.}
}
...
Im Backend wurde das Entwurfsmuster der abstrakten Fabrik zur Realisierung eines Demo-Modus benutzt. Die meisten Kommandos und Kommando-Hilfsmittel gibt es nun nicht nur zur Kommunikation mit dem Raspberry Pi, sondern auch zur "simulierten" Kommunikation unter Einsatz des lokalen Dateisystems. 
...
Sowohl Cache als auch File-Service wurden mit Unter-Packages versehen, so dass die beiden Module nun stärker strukturiert sind. Die Attributierung der Cache-Klassen bzgl. des Timings wurde während der Implementierung stark verändert. Im Entwurf war sie über das gesamte Cache-Modul verteilt. Nun ist sie auf die Cache-Klasse konzentriert. In der Cache-Klasse wurde also etwas mehr an Funktionalität gekapselt als ursprünglich vorgesehen. Auch finden sich viele private Methoden, die im Entwurf zur Deklararierung und Strukturierung rein klasseninterne Funktionalität enthalten waren, sind nur noch spärlich in der Implementierung enthalten. Auch die Struktur und Art der Einbindung von Hilfsklassen hab sich geändert, genauso wie die Platzierung der Run-Methode, die für den Cache-Thread benötigt wird. 
...
Die Cache-Klasse selbst ist der Kern des Cache-Moduls. Hier erwies sich Synchronisierung zu Beginn eines Messlaufs als die größte Herausforderung, und entsprechend gibt hierzu nun eine ganze Reihe von cache-internen Hilfsmethoden. 
