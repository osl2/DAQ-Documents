...
\newglossaryentry{Konfigurationsbaustein}{
	...
	description ={Teil einer Messkonfiguration, der eine Teilaufgabe bestimmten Typs erfüllen kann. Es gibt Sensorbausteine, Transformationsbausteine und Darstellungsbausteine. Liegt am Ausgang eines Bausteins ein Wert an, so kann dieser an den Eingang eines nachgelagerten Bausteins weitergeleitet werden}
}
...
\begin{itemize}
	...
	\item
	\begin{SollKrit}
		Die während eines Messlauf von den Sensor-Bausteinen erfassten Messdaten sollen gespeichert werden können, beispielsweise um diese bei einem weiteren Messlauf als Testdaten nutzen zu können.
	\end{SollKrit}
...
\begin{description}
\hypertarget{link-f050}{\item[F050]} Der Benutzer kann aus einer grafischen Auswahl an vordefinierten \glspl{sensor} einzelne Sensoren ansprechen.
\hypertarget{link-f060}{\item[F060]} Der Benutzer kann aus einer grafischen Auswahl an vordefinierten \glspl{transformation} einzelne Transformationen ansprechen.
\hypertarget{link-f070}{\item[F070]} Der Benutzer kann aus einer grafischen Auswahl an vordefinierten \glspl{darstellung} einzelne Darstellungen ansprechen.
...
\hypertarget{link-f115}{\item[(opt.) F115]} Der Benutzer soll in der graphischen Auswahl an Darstellungen (\hyperlink{link-f070}{F075}) eigene Darstellungen hinzufügen können.
\end{description}
...
\begin{tabular}{ | c | c | c | c | }
	\hline
	\textbf{Merkmal} & \textbf{sehr wichtig} & \textbf{wichtig} & \textbf{weniger wichtig} \\
	\hline
	Robustheit & X & & \\
